...
const calculateRegionPrice = (region: Region) => {
  let edges = 0;
  for (const pos of region.plots.values()) {
    ...
    for (const neighbourPos of neighbourPositions) {
      // count edges that are not part of the region
      if (!region.plots.has(serializeXY(neighbourPos.x, neighbourPos.y))) {
        edges++;
      }
    }
  }
  return edges * region.plots.size;
};

const calculateTotalPrice = (regions: Array<Region>) => {
  const result = regions.reduce((acc, cur) => {
    return acc + calculateRegionPrice(cur);
  }, 0);
...
const part1 = (input: string) => {
  ...
  const result = calculateTotalPrice(regions);
  return result; //?
};
